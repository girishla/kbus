...
import com.expensemanager.app.tripsheet.filter.CategoryFilterFragment;
import com.expensemanager.app.tripsheet.filter.DateFilterFragment;
import com.expensemanager.app.tripsheet.filter.MemberFilterFragment;
...
public class TripSheetActivity extends BaseActivity {

    private static final String TAG = TripSheetActivity.class.getSimpleName();
    ...
    private com.expensemanager.app.tripsheet.TripSheetAdapter expenseAdapter;
    ...
    public static void newInstance(Context context) {
        Intent intent = new Intent(context, TripSheetActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        expenseAdapter = new TripSheetAdapter(this, expenses);
        setupRecyclerView();
        setupSwipeToRefresh();

        fab.setOnClickListener(v -> {
            NewTripSheetActivity.newInstance(this);
            overridePendingTransition(R.anim.right_in, R.anim.stay);
        });

        if (isCategoryFiltered && category != null) {
            TripSheetActivity.this.toolbar.setBackgroundColor(Color.parseColor(category.getColor()));
        }
    }
    ...
    private void invalidateViews() {
        ...
        if (isMemberFiltered && isCategoryFiltered && isDateFiltered) {
            expenseAdapter.addAll(BusDailySummary.getAllTripSheetsByMemberAndDateAndCategoryAndGroupId(member, startDate, endDate, category, groupId));
        } else if (isMemberFiltered && isDateFiltered) {
            expenseAdapter.addAll(BusDailySummary.getAllTripSheetsByMemberAndDateAndGroupId(member, startDate, endDate, groupId));
        } else if (isMemberFiltered && isCategoryFiltered) {
            expenseAdapter.addAll(BusDailySummary.getAllTripSheetsByMemberAndCategoryAndGroupId(member, category, groupId));
        } else if (isCategoryFiltered && isDateFiltered) {
            expenseAdapter.addAll(BusDailySummary.getAllTripSheetsByDateAndCategoryAndGroupId(startDate, endDate, category, groupId));
        } else if (isMemberFiltered) {
            expenseAdapter.addAll(BusDailySummary.getAllTripSheetsByMemberAndGroupId(member, groupId));
        } else if (isCategoryFiltered) {
            expenseAdapter.addAll(BusDailySummary.getAllTripSheetsByCategoryAndGroupId(category, groupId));
        } else if (isDateFiltered) {
            expenseAdapter.addAll(BusDailySummary.getAllTripSheetsByDateAndGroupId(startDate, endDate, groupId));
        } else {
            expenseAdapter.addAll(BusDailySummary.getAllTripSheetsByGroupId(groupId));
        }

        if (Helpers.needToSync(syncTimeInMillis)) {
            SyncTripSheet.getAllTripSheetsByGroupId(groupId);
    ...
    private void setupSwipeToRefresh() {
        swipeContainer.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                SyncTripSheet.getAllTripSheetsByGroupId(groupId).continueWith(onGetTripSheetFinished, Task.UI_THREAD_EXECUTOR);
            }
        });
    ...
    private MemberFilterFragment.MemberFilterListener memberFilterListener = new MemberFilterFragment.MemberFilterListener() {
        ...
        public void onFinishMemberFilterDialog(Member member) {
            if (!isMemberFiltered ||
                (TripSheetActivity.this.member != null && member != null && TripSheetActivity.this.member.getId().equals(member.getId()))) {
                isMemberFiltered = !isMemberFiltered;
            }

            TripSheetActivity.this.member = member;
    ...
    private DateFilterFragment.DateFilterListener dateFilterListener = new DateFilterFragment.DateFilterListener() {
        ...
        public void onFinishDateFilterDialog(Date startDate, Date endDate) {
            ...
            TripSheetActivity.this.startDate = startDate;
            TripSheetActivity.this.endDate = endDate;
            invalidateViews();
        }
    };
    ...
    private CategoryFilterFragment.CategoryFilterListener categoryFilterListener = new CategoryFilterFragment.CategoryFilterListener() {
        ...
        public void onFinishCategoryFilterDialog(Category category) {
            if (!isCategoryFiltered ||
                    ((TripSheetActivity.this.category == null && category == null) || (TripSheetActivity.this.category != null
                            && category != null && TripSheetActivity.this.category.getId().equals(category.getId())))) {
                isCategoryFiltered = !isCategoryFiltered;
            }
            TripSheetActivity.this.category = category;
